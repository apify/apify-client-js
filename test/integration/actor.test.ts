...
async function createActor(options: { title?: string; withVersion?: boolean } = {}): Promise<Actor> {
...
test('actors().list({ my: true }) returns only the test user Actors', async () => {
...
test('lastRun() resolves to the run that was just made', async () => {
 ...
 * through these local shapes. They are worth pinning now: `feat/openapi-generated-models` will make
 * the field names load-bearing, and this is the tier that would catch an alias being dropped.
 */
