...
const CROSS_USER_PERMISSION_ERROR = /Insufficient permissions for the dataset/;
...
test('datasets().list() iterates the user datasets across pages', async () => {
    ...
    try {
        const collected = await collectUntilPresent(() => client.datasets().list({ desc: true }), createdIds);
...
test('pushItems() round-trips a payload large enough to be compressed', async () => {
    ...
    try {
        ...
        // exercises the brotli path end to end against the API.
...
describe('a dataset owned by another user', () => {
    test('listItems() is rejected without a signature and succeeds with one', async () => {
        const fixture = unwrapFixture(inject('crossUserDataset'));
        const datasetClient = client.dataset(fixture.id);

        await expect(datasetClient.listItems()).rejects.toThrow(CROSS_USER_PERMISSION_ERROR);

        const page = await datasetClient.listItems({ signature: fixture.signature });
        expect(page.items).toEqual(fixture.expectedContent);
    });

    test('listItems() iteration is rejected without a signature and succeeds with one', async () => {
        const fixture = unwrapFixture(inject('crossUserDataset'));
        const datasetClient = client.dataset(fixture.id);

        await expect(async () => {
            for await (const _item of datasetClient.listItems()) {
                // The rejection happens on the first page, before anything is yielded.
            }
        }).rejects.toThrow(CROSS_USER_PERMISSION_ERROR);

        const collected: Record<string, any>[] = [];
        for await (const item of datasetClient.listItems({ signature: fixture.signature })) {
            collected.push(item);
        }
        expect(collected).toEqual(fixture.expectedContent);
    });

    test('downloadItems() is rejected without a signature and succeeds with one', async () => {
        const fixture = unwrapFixture(inject('crossUserDataset'));
        const datasetClient = client.dataset(fixture.id);

        await expect(datasetClient.downloadItems(DownloadItemsFormat.JSON)).rejects.toThrow(
            CROSS_USER_PERMISSION_ERROR,
        );

        const downloaded = await datasetClient.downloadItems(DownloadItemsFormat.JSON, {
            signature: fixture.signature,
        });
        expect(JSON.parse(downloaded.toString('utf8'))).toEqual(fixture.expectedContent);
    });
});
