...
test('tasks().list() contains a freshly created task', async () => {
    ...
    try {
        const collected = await collectUntilPresent(() => client.tasks().list({ limit: 100 }), [createdTask.id]);
...
test('tasks().list() iterates the user tasks across pages', async () => {
    ...
    try {
        const collected = await collectUntilPresent(() => client.tasks().list({ desc: true }), createdIds);
        const collectedIds = new Set(collected.map((item) => item.id));
