...
import { makeClient, unwrapFixture } from './_fixtures.js';
import { collectUntilPresent, getRandomResourceName, pollUntilCondition } from './_utils.js';

const CROSS_USER_PERMISSION_ERROR = /Insufficient permissions for the key-value store/;
...
test('keyValueStores().list() iterates the user stores across pages', async () => {
    ...
    try {
        const collected = await collectUntilPresent(() => client.keyValueStores().list({ desc: true }), createdIds);
...
describe('a key-value store owned by another user', () => {
    test('listKeys() is rejected without a signature and succeeds with one', async () => {
        const fixture = unwrapFixture(inject('crossUserKvs'));
        const storeClient = client.keyValueStore(fixture.id);

        await expect(storeClient.listKeys()).rejects.toThrow(CROSS_USER_PERMISSION_ERROR);

        const result = await storeClient.listKeys({ signature: fixture.signature });
        expect(result.items.map((item) => item.key).sort()).toEqual(Object.keys(fixture.expectedContent).sort());
    });

    test('getRecord() is rejected without a signature and succeeds with the per-key one', async () => {
        const fixture = unwrapFixture(inject('crossUserKvs'));
        const storeClient = client.keyValueStore(fixture.id);
        const key = 'key1';

        await expect(storeClient.getRecord(key)).rejects.toThrow(CROSS_USER_PERMISSION_ERROR);

        const record = await storeClient.getRecord(key, { signature: fixture.keysSignature[key] });
        expect(record?.value).toBe(fixture.expectedContent[key]);
    });

    test('getRecord() with the buffer option is rejected without a signature and succeeds with one', async () => {
        const fixture = unwrapFixture(inject('crossUserKvs'));
        const storeClient = client.keyValueStore(fixture.id);
        const key = 'key1';

        await expect(storeClient.getRecord(key, { buffer: true })).rejects.toThrow(CROSS_USER_PERMISSION_ERROR);

        const record = await storeClient.getRecord(key, {
            buffer: true,
            signature: fixture.keysSignature[key],
        });
        expect(JSON.parse(record!.value.toString('utf8'))).toBe(fixture.expectedContent[key]);
    });

    test('getRecord() with the stream option is rejected without a signature and succeeds with one', async () => {
        const fixture = unwrapFixture(inject('crossUserKvs'));
        const storeClient = client.keyValueStore(fixture.id);
        const key = 'key1';

        await expect(storeClient.getRecord(key, { stream: true })).rejects.toThrow(CROSS_USER_PERMISSION_ERROR);
        ...
        const body = await readStream(record!.value);
        expect(JSON.parse(body.toString('utf8'))).toBe(fixture.expectedContent[key]);
    });
});
