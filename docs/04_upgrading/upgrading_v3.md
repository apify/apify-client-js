...
`ApifyClient` sends its requests through an HTTP client you can replace. <ApiLink to="class/HttpClient">`HttpClient`</ApiLink> is now the abstract base holding the shared request pipeline, and the axios-based client the `ApifyClient` uses by default is <ApiLink to="class/AxiosHttpClient">`AxiosHttpClient`</ApiLink>. A custom client extends `HttpClient`, implements `sendRequest()` and is plugged in with <ApiLink to="class/ApifyClient#withCustomHttpClient">`ApifyClient.withCustomHttpClient()`</ApiLink>. See [HTTP clients](../02_concepts/08_http-clients.md) for the contract. Decoupling the pipeline from axios changes a few details of the public surface.
...
The method is reported as the client sent it, so `POST` where v2 reported axios's lowercased `post`. <ApiLink to="class/ResponseValidationError">`ResponseValidationError`</ApiLink> takes its method from the same place, so its message is uppercase too.
