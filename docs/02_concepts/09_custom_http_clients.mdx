...
import DefaultHttpClientExample from '!!raw-loader!./code/09_default_http_client.ts';
import AxiosInterceptorsExample from '!!raw-loader!./code/09_axios_interceptors.ts';
import ArchitectureImportsExample from '!!raw-loader!./code/09_architecture_imports.ts';
import PluggingInExample from '!!raw-loader!./code/09_plugging_in.ts';
...
- Configurable [timeout tiers](./10_timeouts.mdx) that grow with every retry.
...
- `isRetryableTransportError(error)` classifies transport failures for the shared retry loop. The default classifies nothing as retryable, so a transport that doesn't override it gives up on the first connection failure.
...
- `close()` releases resources owned by the transport, such as a connection pool. The default doesn'thing.
...
- **Route through a proxy** - Add proxy support or request routing the environment variables can't express.
...
For a complete implementation over `fetch`, see [Build a custom HTTP client](../03_guides/05_custom_http_client.mdx). The <ApiLink to="class/HttpClient">`HttpClient`</ApiLink> API reference documents the full contract.
